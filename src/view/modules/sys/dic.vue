  ...
  <div class="search">
    <Card>
      <Row>
        <Form ref="searchForm" :model="searchForm" inline :label-width="90" class="search-form">
          <Form-item label="类别" prop="dicType">
            <Select v-model="searchForm.dicType" filterable clearable style="width:170px" @on-change="handleSearch">
              <Option v-for="item in dicTypeList" :value="item.value" :key="item.value">{{ item.name }}</Option>
            </Select>
          </Form-item>
          <Form-item label="名称" prop="dicName">
            <Input type="text" v-model="searchForm.dicName" clearable placeholder="请输入名称" style="width: 170px" />
          </Form-item>
          <FormItem label="是否启用" prop="enable">
            <i-switch v-model="searchForm.enable" size="large">
        ...
        <Button v-if="isAuth('sys:alldic:delete')" @click="deleteData">批量删除</Button>
        </Col>
      </Row>
      <Row>
        <tables :loading="loading" ref="table" size="small" border stripe v-model="pageData.list" :columns="columns" @on-selection-change="selectionChangeHandle" />
      </Row>
    ...
    <Modal :title="modalTitle" v-model="formModalVisible" :mask-closable='false' :width="500" class-name="vertical-center-modal">
      <Form ref="dataForm" :model="dataForm" :label-width="80" :rules="dataFormValidate">
        <FormItem label="类别" prop="dicType">
          <Select v-model="dataForm.dicType" >
            <Option v-for="item in dicTypeList" :value="item.value" :key="item.value">{{ item.name }}</Option>
          </Select>
        </FormItem>
        ...
        <FormItem label="编码" prop="dicValue">
...
export default {
  ...
  mounted () {
    this.dicTypeList = dicUtil.getDicTypeList()
    this.init()
  },
  methods: {
    init () {
      ...
      api.getDicList(this.searchForm).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.pageData.list.map(item => {
            ...
            item.dicTypeStr = dicUtil.getDicTypeName(item.dicType)
          })
        }
      })
    },
    ...
    addData () {
      ...
      this.dataForm.dicType = this.searchForm.dicType
