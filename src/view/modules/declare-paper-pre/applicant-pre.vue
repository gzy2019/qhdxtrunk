      ...
      <Row>
        ...
        >
          ...
          <template slot-scope="{ row }" slot="openFlag9">
            <Tag :color="`${row.openFlag9 ? 'primary' : 'error' }`">{{ row.openFlag8 ? '是' : '否' }}</Tag>
          </template>
...
<script>
import api from '@/api/mpad-pre/mpadapplicantpre'
import projectApi from '@/api/mpad-pre/mpadprojectpre'
import IDatePicker from '@/view/components/idate-picker'
export default {
  name: 'MpadApplicantPre-manage',
    ...
    saveMpadApplicant () {
      this.$refs.dataForm.validate(valid => {
        if (valid) {
          ...
          if (!this.dataForm.id) {
            api
              .saveMpadApplicant(this.dataForm)
              .then(res => {
            ...
            api
              .updateMpadApplicant(this.dataForm)
              .then(res => {
                this.submitLoading = false
                this.$Message.success('操作成功！')
                this.formModalVisible = false
