    ...
    <div class="search">
        <Card>
            ...
            <Row class="operation btn_group">
                <Col span="24">
                  <Button v-if="isAuth('paypoint:paypointpubservice:save')" @click="addPaypointPubService" type="primary" icon="md-add">新增</Button>
                  <Button v-if="isAuth('paypoint:paypointpubservice:delete')" :disabled="selectedList.length === 0" @click="deletePaypointPubService()" type="primary" icon="ios-trash-outline">删除</Button>
            ...
            <Row>
                <Table :loading="loading" ref="table" size="small" border stripe :data="pageData.list" :columns="columns" @on-selection-change="selectionChangeHandle">
                    ...
                    <template slot-scope="{ row }" slot="payNotCounted">
                        <Tag :color="row.payNotCounted?'primary':'default'">{{row.payNotCounted ? '是' : '否'}}</Tag>
                    </template>
                    <template slot-scope="{ row }" slot="action">
                        <ButtonGroup>
                            <Button v-if="isAuth('paypoint:paypointpubservice:info')"   type="primary" size="small" @click="viewPaypointPubService(row)">查看</Button>
                            <Button v-if="isAuth('paypoint:paypointpubservice:update')" type="primary" size="small" @click="editPaypointPubService(row)">编辑</Button>
                            <Button v-if="isAuth('paypoint:paypointpubservice:delete')" type="error"   size="small" @click="deletePaypointPubService(row)">删除</Button>
                        </ButtonGroup>
                    </template>
                </Table>
            </Row>
        ...
        <PaypointPubServiceForm ref="paypointPubServiceForm" @update="getPaypointPubServiceList" />
        <PaypointPubServiceEditForm ref="paypointPubServiceEditForm" @update="getPaypointPubServiceList" />
        <PaypointPubServiceView ref="paypointPubServiceView" />

        <ExcelUpload ref="excelUpload" @on-upload="importRecord" @on-load="preImportHandle" :inTemplate="{name: '公共服务导入模板', url: 'http://info.sppm.tsinghua.edu.cn/sppmtest/file/template/paypoint-pub.xlsx'}"/>
    </div>
</template>
...
import PaypointPubServiceForm from './components/paypointpubservice-form'
import PaypointPubServiceEditForm from './components/paypointpubservice-editform'
import PaypointPubServiceView from './components/paypointpubservice-view'
import ExcelUpload from '@/view/components/excel-upload'
import UploadUtil from './components/paypoint-upload'
export default {
  name: 'PaypointPubService-manage',
  components: { PaypointPubServiceForm, PaypointPubServiceEditForm, PaypointPubServiceView, ExcelUpload },
  data () {
    return {
      loading: true,
  ...
  methods: {
    init () {
      this.getPaypointPubServiceList()
      this.adjustTableHeight()
      this.loadBatchList()
    },
    loadBatchList () {
      batchapi.getPaypointBatchDataList({ category: 1, pubserviceBatch: 1 }).then(res => {
    ...
    getPaypointPubServiceList () {
      this.loading = true
      api.getPaypointPubServiceList(this.searchForm).then(res => {
        this.loading = false
        this.selectedList = []
    ...
    exportData () {
      ...
      api.getPaypointPubServiceList(params).then(res => {
        let data = res.data
    ...
    importRecord (records, fileName) {
      ...
      UploadUtil.importPubService(records, this.$refs.excelUpload).then(() => {
        this.handleSearch()
      }, (msg) => {
    ...
    addPaypointPubService () {
      this.$refs.paypointPubServiceForm.addPaypointPubService()
    },
    viewPaypointPubService (v) {
      this.$refs.paypointPubServiceView.viewPaypointPubService(v.id)
    },
    editPaypointPubService (v) {
      this.$refs.paypointPubServiceEditForm.editPaypointPubService(v.id)
    },
    selectionChangeHandle (list) {
      this.selectedList = list
    },
    deletePaypointPubService (row) {
      let ids = row ? [row.id] : this.selectedList.map(item => item.id)
      this.$Modal.confirm({
        title: '确认提示',
        content: `<p>您确定删除${ids.length > 1 ? ids.length + '条记录' : '此记录'}吗?</p>`,
        onOk: () => {
          api.deletePaypointPubService(ids).then(res => {
