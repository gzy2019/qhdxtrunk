  ...
  <div class="search">
    <Card>
      <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
        <Form-item label="岗位名称" prop="postName">
          <Input v-model="searchForm.postName" :clearable="true" ></Input>
        </Form-item>
        <Form-item label="岗位部门" prop="departmentId">
          <DepartmentSelect v-model="searchForm.departmentId" clearable></DepartmentSelect>
        </Form-item>
      ...
      <Row>
        <Form ref="searchFormRecord" :model="searchFormRecord" inline :label-width="100" class="search-form">
          <Col span="16">
            ...
            <Form-item label="学号" prop="stuCode">
              <Input v-model="searchFormRecord.stuCode" :clearable="true" style="width: 200px"></Input>
            </Form-item>
        ...
        <Col span="24" style="margin-bottom:10px">
          ...
          <Button @click="modalExport = true">导出</Button>
        </Col>
      </Row>
      <Row>
    ...
    <Modal title="" v-model="modalSelected" class-name="vertical-center-modal popup">
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="modalSelected = false">确定</Button>
      </div>
    </Modal>
    <!--选定 end-->
    <!--导出 start-->
    <Modal title="" v-model="modalExport" class-name="vertical-center-modal popup">
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="modalExport = false">确定</Button>
      </div>
    </Modal>
    <!--导出 end-->
    <!--拒绝 start-->
    <Modal title="" v-model="modalRefuse" class-name="vertical-center-modal popup">
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="modalRefuse = false">确定</Button>
      </div>
    </Modal>
    <!--拒绝 end-->
    ...
    <Modal title="申报详情" v-model="modalDetail" footer-hide class-name="vertical-center-modal popup" :width="600" scrollable>
      <div class="baseinformation">
        <div class="popup_title">
          <p>
            <Icon type="ios-list" />岗位信息</p>
          <Divider />
        </div>
        <div class="popup-content" style="margin-bottom:10px">
          <table class="table1">
            <tr>
              <td>岗位名：{{dataForm.postName}}</td>
              <td>设岗部门：{{dataForm.postType}}</td>
              <td>岗位数量：{{dataForm.postCount}}</td>
            </tr>
            <tr>
              <td>学生类别：{{dataForm.postAdmin && dataForm.postAdmin.typeListStr}}</td>
              <td>负责人：{{dataForm.teacherName}}</td>
            </tr>
            <tr>
              <td colspan="3">岗位性质：{{dataForm.postType}}</td>
            </tr>
            <tr>
              <td colspan="3">岗位工作内容：{{dataForm.postDesc}}</td>
            </tr>
            <tr>
              <td colspan="3">岗位要求：{{dataForm.postCondition}}</td>
            </tr>
          </table>
        </div>
        <div class="popup_title">
        ...
        <div class="popup-content" style="margin-bottom:10px">
          <table class="table1">
            ...
            <tr>
              <td colspan="3">邮箱：{{dataForm.student && dataForm.student.email}}</td>
            </tr>
          </table>
        </div>
...
export default {
  ...
  methods: {
    ...
    getPostAdminList () {
      ...
      api.getOwnerPostAdminList(this.searchForm).then(res => {
        this.loading = false
        this.selectedList = []
        if (res.code === 200) {
      ...
      apiRecord.getRecordList(this.searchFormRecord).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.pageDataRecord.list.map(item => {
            ...
            item.postDesc = item.postAdmin && item.postAdmin.postDesc
            ...
            console.log(item.recordStatus, 'recordStatus')
