    ...
    <Card>
      <Form ref="searchForm" :model="searchForm" @keydown.enter.native="handleSearch" inline :label-width="60" class="search-form">
        <Form-item label="成果名称" prop="name">
          <Input type="text" v-model="searchForm.name" clearable placeholder="请输入成果名称" style="width: 200px" />
        </Form-item>
        ...
        <Form-item label="第一作者" prop="firstAuthor">
          <Input type="text" v-model="searchForm.firstAuthor" clearable placeholder="请输入第一作者" style="width: 200px" />
        </Form-item>
        <FormItem label="成果来源" prop="source">
          <DicSelect v-model="searchForm.source" style="width: 200px" dtype="achievement-source" :clearable="true" placeholder="请选择成果来源" />
        </FormItem>
        <FormItem label="学科门类" prop="subject">
          <DicSelect v-model="searchForm.subject" style="width: 200px" dtype="achievement-subject" :clearable="true" placeholder="请选择学科门类" />
        </FormItem>
        <FormItem label="发表范围" prop="scope">
            <DicSelect v-model="searchForm.scope" style="width: 200px" dtype="achievement-scope" :clearable="true" placeholder="请选择发表范围" />
          </FormItem>
        <Form-item style="margin-left: -70px;">
          <Button @click="handleSearch" type="primary" icon="ios-search" style="margin-left:10px">搜索</Button>
          <Button @click="handleClear" style="margin-left: 5px">清除</Button>
        </Form-item>
      </Form>
      <Row class="operation btn_group">
        <i-Col span="24">
        <Button v-if="isAuth('achievement:achievementinfo:save')" @click="addAchievementInfo" type="primary" icon="md-add">新增</Button>
        <Button v-if="isAuth('achievement:achievementinfo:delete')" :disabled="selectedList.length === 0" @click="deleteAchievementInfo()" type="primary" icon="ios-trash-outline">删除</Button>
      ...
      <Row>
        <Table :loading="loading" ref="table" size="small" border stripe :data="pageData.list" :columns="columns" @on-selection-change="selectionChangeHandle">
          <template slot-scope="{ row }" slot="action">
            <ButtonGroup>
              <Button v-if="isAuth('achievement:achievementinfo:info')" type="primary" size="small" @click="viewAchievementInfo(row)">查看</Button>
              <Button v-if="isAuth('achievement:achievementinfo:update')" type="primary" size="small" @click="editAchievementInfo(row)">编辑</Button>
              <Button v-if="isAuth('achievement:achievementinfo:delete')" type="error" size="small" @click="deleteAchievementInfo(row)">删除</Button>
            </ButtonGroup>
          </template>
        </Table>
      </Row>
...
export default {
  ...
  methods: {
    ...
    getAchievementInfoList () {
      this.loading = true
      api.getMyAchievementInfoList(this.searchForm).then(res => {
        this.loading = false
        this.selectedList = []
    ...
    exportAchievementInfo () {
      ...
      api.getMyAchievementInfoList(params).then(res => {
